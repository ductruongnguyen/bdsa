...
from shopee_initial_load import (CategoryETL as InitialCategory, ProductETL as InitialProduct,
                                 LocationETL as InitialLocation, FactSaleETL as InitialFactSale)
...
class CategoryIncrementalETL(BaseETL):
    def process_data(self, spark: SparkSession, new_categories_df: DataFrame) -> Any:
        shopee_platform_id = PLATFORM['shopee']
        categories_db = read_data_from_postgres(spark, 'dim_category')
        categories_db = categories_db.filter(F.col('platform_id') == F.lit(shopee_platform_id))
...
def runner():
    ...
    shopee_data_path = "./data/shopee/*.json"
    shopee_data = (spark.read
                   .option("multiline", "true")
                   .json(shopee_data_path))

    # dim_category incremental load
    new_categories_df = InitialCategory().process_data(spark, shopee_data)
    ...
    new_locations_df = InitialLocation().process_data(spark, shopee_data)
    if new_locations_df.count() > 0:
        InitialLocation().write_data_to_postgres(spark, new_locations_df, 'dim_location')

    # dim_product incremental load
    new_products_df = InitialProduct().process_data(spark, shopee_data)
    ...
    new_fact_sale_df = InitialFactSale().process_data(spark, shopee_data)
    ...
    fact_periodic_df = FactPeriodicIncrementalETL().process_data(spark, PLATFORM['shopee'])
